...
const NAV = [
  { href: "/volunteer/dashboard", label: "MY PEOPLE" },
...
export function VolunteerHeader({ name, email }: Props) {
    ...
    <header className="relative z-30 border-b border-border bg-ink">
      <div className="container flex max-w-6xl items-center justify-between gap-3 py-4">
        ...
        <div className="flex items-center gap-2 text-sm md:gap-3">
          <span
            className="hidden max-w-[220px] truncate text-cream-40 lg:inline lg:max-w-none"
            title={email}
          >
            {name} · {email}
          </span>
        ...
        <>
          ...
          <div className="absolute left-0 right-0 top-full z-40 border-b border-border bg-ink shadow-lg md:hidden">
            <nav className="container max-w-6xl py-2">
              ...
              <div className="mt-1 flex flex-wrap items-center justify-between gap-2 border-t border-border px-2 py-3">
                <span className="truncate text-xs text-cream-40" title={email}>
                  {name} · {email}
                </span>
                <VolunteerLogoutButton />
              </div>
